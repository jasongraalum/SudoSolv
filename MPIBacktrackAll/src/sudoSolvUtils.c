...
Puzzle * loadPuzzle(char *filename)
{
    ...
    p->branch_done = (SolNode *)malloc(sizeof(SolNode));
    p->branch_done->row = -1;
    p->branch_done->col = -1;
    p->branch_done->next_cells = NULL;
...
SolNode * addNodes(Puzzle *p, Cell * s, SolNode *node, int pos)
{
    ...
    // Error condition - position is not at the top, but the node is NULL
    if(node == NULL && pos != 0) return(NULL); 

    // Set all next cells to dead_end and return NULL if at the bottom
    if(row == p->degree && node != NULL) {
        for(int i = 0; i < p->degree+1; i++) {
            node->next_cells[i] = p->dead_end;
        }
        return(NULL);
    }
    ...
    if(node == NULL) {
        ...
        // If the first cell is fixed, set all others to dead_end
        if(p->fixed_cells[0] != 0) {
            for(unsigned int i = 0; i < p->degree+1; i++)
                if(i != val) node->next_cells[i] = p->dead_end;
        }
    }

    // Invalid solution branch <- should never happen
    if(node->next_cells[val] == p->dead_end)
        return(p->dead_end);

    // Add node if the next cell is NULL
    else if(node->next_cells[val] == NULL) {
        node->next_cells[val] = createNode(p, pos+1);

        // If this is a fixed cell, set all others to dead_end
