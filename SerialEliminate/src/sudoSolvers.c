...
int elimSolve(Puzzle * p, int numThreads)
{
    int totalProcs = 3*p->degree;
    int procsPerThread = totalProcs/numThreads;

    printPuzzle(p);
    for(int row = 0; row < p->degree; row++) {
        for(int col = 0; col < p->degree; col++) {
            if(getCell(p, row, col) == 0) setCell(p, row, col, p->degree+1);
        }
    }
