...
int checkPuzzleCol(Puzzle * p, Cell * s, int col)
{
    for(unsigned int row1 = 0; row1 < p->degree-1; row1++) {
        for(unsigned int row2 = row1+1; row2 < p->degree; row2++) {
            if(s[row1*p->degree+col] == s[row2*p->degree+col] && s[row1*p->degree+col] != 0) {
...
int checkPuzzleRow(Puzzle * p, Cell * s, int row)
{
    for(unsigned int col1 = 0; col1 < p->degree-1; col1++) {
        for(unsigned int col2 = col1+1; col2 < p->degree; col2++) {
            if(s[row*p->degree+col1] == s[row*p->degree+col2] && s[row*p->degree+col1] != 0) {
                return(-1);
            }
        }
    }
...
int checkPuzzleBlock(Puzzle *p, Cell * s, int k, int l) 
{
    int deg_sqrt = (int)sqrt(p->degree);
    for(unsigned int c1 = 0; c1 < p->degree-1; c1++) {
        unsigned int col1 = k*deg_sqrt + (c1 % deg_sqrt);
        unsigned int row1 = l*deg_sqrt + (c1 / deg_sqrt);
        for(unsigned int c2 = c1+1; c2 < p->degree; c2++) {
            unsigned int col2 = k*deg_sqrt + (c2 % deg_sqrt);
            unsigned int row2 = l*deg_sqrt + (c2 / deg_sqrt);
            if(s[row1*p->degree + col1] == s[row2*p->degree + col2] && s[row1*p->degree + col1] != 0) {
                return (-1);
            }
        }
    }
...
int verifySolution(Puzzle *p, Cell * s)
{
    // Check rows
    // For all rows
    ...
    // For all columns  
    for(unsigned int row = 0; row < p->degree; row++) {
        if(checkPuzzleRow(p, s, row) < 0) return(-1);
    }
    ...
    // For all rows  
    for(unsigned int col = 0; col < p->degree; col++) {
        if(checkPuzzleCol(p, s, col) < 0) return(-1);
    }
    ...
    for(unsigned int k = 0; k < deg_sqrt; k++) {
        for(unsigned int l = 0; l < deg_sqrt; l++) {
            if(checkPuzzleBlock(p, s, k, l) < 0) return(-1);
        }
    }
    return(1);
}
